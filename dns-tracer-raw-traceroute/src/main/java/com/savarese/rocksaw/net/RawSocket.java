...
public class RawSocket {
    ...
    private int __getScopeId(InetAddress address) {
        if (__family == PF_INET6 && address instanceof Inet6Address) {
    ...
    private native static int __query_routing_interface(int socket, int family,
     ...
     * @throws IllegalStateException If the object instance is
     *                               already open.
     * @throws IOException           If an error occurs while opening the socket.
     */
     ...
     * @throws IllegalStateException If the socket has not been opened first.
    ...
    public void bind(InetAddress address)
        ...
        if (__bind(__socket, __family, address.getAddress(), scope_id) != 0) {
            __throwIOException();
        }
    }
     ...
     *               cannot be bound.
     * @throws IllegalStateException         If the socket has not been opened first.
     * @throws UnsupportedOperationException If binding a device
     *                                       name is not supported on the runtime platform.
     * @throws IOException                   If the device cannot be bound.
     */
     ...
     * @throws SocketException If the option setting could not be altered.
     */
     ...
     * @throws SocketException If the option value could not be retrieved.
     */
     ...
     * @throws SocketException If the option value could not be set.
     */
     ...
     */
    private native static int __select(int socket, boolean read, int milliseconds);
    ...
    private native static int __setTTL(int socket, int value);
     ...
     * @throws SocketException If the option value could not be set.
     */
    public void setTTL(int value) throws SocketException {

        int result = __setTTL(__socket, value);
     ...
     * @throws IllegalArgumentException If the offset or lengths are
     *                                  invalid or if the address parameter is the wrong length.
     * @throws IOException              If an I/O error occurs.
     * @throws InterruptedIOException   If the read operation times out.
     */
     ...
     * @throws IllegalArgumentException If the offset or lengths are invalid.
     * @throws IOException              If an I/O error occurs.
     * @throws InterruptedIOException   If the write operation times out.
     */
